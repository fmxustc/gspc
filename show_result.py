...
# Gini Coefficient
rg = sns.distplot(data['G1'].values, bins=100, color='r', ax=ar[0, 0], hist=False, kde=True, label='type1')
rg.set(xlabel='Gini Coefficient', ylabel='Gaussian Kernel Density')

sns.distplot(data['G2'].values, bins=100, color='b', ax=ar[0, 0], hist=False, kde=True, label='type2')
...
#  Moment Index
# print(len(data[(data.M1 < 0.015)]), len(data[(data.M2 < 0.015)]))
rm = sns.distplot(data[data.M1 < 0.015]['M1'].values, bins=100, color='g', ax=ar[1, 0], hist=False, kde=True, label='type1')
rm.set(xlabel='Moment Index', ylabel='Gaussian Kernel Density')
sns.distplot(data[(data.M2 < 0.015) & (data.M2 > 0)]['M2'].values, bins=100, color='c', ax=ar[1, 0], hist=False, kde=True, label='type2')
